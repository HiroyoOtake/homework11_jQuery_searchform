...
$num = $_POST['pref'];
$pref = $prefectures[$num];
  $count = 0;
if (isset($places[$num])) {
  $count = count($places[$num]);
} 
//var_dump($num);
//var_dump($places[$num]);
?>
      ...
      <div class="form-group">
        <select name="pref" class="form-control">
         <?php foreach ($prefectures as $number => $prefecture): ?> 
          <option value="<?php echo $number; ?>" <?php if($num == $number): ?>selected<?php endif ?>><?php echo $prefecture; ?></option>
         <?php endforeach ?>
        </select>
      </div>
...
<?php if ($count == 0): ?>
 <p class="search-result"><?php echo $pref; ?>の観光スポットは見つかりませんでした。</p>
<?php else: ?>
 <p class="search-result"><?php echo $pref; ?>の観光スポットは<?php echo $count; ?> 件見つかりました。</p>
  <?php foreach($places[$num] as $value): ?>
    <div class="media">
